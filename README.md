...
診斷先比對已知失敗樣態（契約違反、抽取失敗、存取受阻、時效不符）；能判定就直接分類，否則再交給模型。分類後決定路由：可修復的進修復迴圈，不可修復的記錄為待處理。

分類必須明確。例如 401 是權限或入口問題；找不到文章則可能是網址錯誤。混在一起會導致錯誤的排查方向。

### 驗證 + 修復迴圈

某次抓取 BBC 時，明細樣本誤抓成導覽頁，導致模型從錯誤證據學習抽取規則，後續兩輪修復也持續失敗。

因此把前期偵查改成子迴圈：證據不正確就更換抓取方式重試，驗證通過後才往下執行。
...
展示介面貼一個網址啟動，逐關顯示誰在做什麼。預設只跑前期偵查，不花錢。
