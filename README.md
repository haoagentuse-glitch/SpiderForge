...
可從 `workspace/backend` 執行以下指令查看目前指標：

```console
../.venv/Scripts/python.exe -m app.spider_forge_system.runs.ledger
```
...
```text
spider_forge_system/
├── __main__.py               # python -m app.spider_forge_system
├── cli.py                    # 所有命令列子命令
├── pipeline.py
├── config.py
├── state.py
...
`SPIDERFORGE_DATA_DIR` 指向 `workspace/runtime/spider_forge`：
...
先進入 `workspace/backend`。專案的 `workspace/.venv` 由 UV 管理，因此 UV 只負責
安裝與檢查相依套件，程式、編譯與測試都直接交給該環境的 Python；不需要啟用環境，
也不依賴專案自訂 PowerShell 腳本：

```console
uv pip install --python ../.venv/Scripts/python.exe -r app/spider_forge_system/requirements-test.txt

uv pip check --python ../.venv/Scripts/python.exe
../.venv/Scripts/python.exe -m compileall -q app/spider_forge_system crawler_runtime/news_crawler
../.venv/Scripts/python.exe -m pytest -q app/spider_forge_system/tests crawler_runtime/tests
```
...
`workspace/.venv/Scripts/python.exe`，後續可把 `../.venv/Scripts/python.exe`
簡寫成 `python`。
...
真實 pipeline 會存取網站並呼叫模型，可能消耗外部 API 額度。請先在 `workspace/.env`
或作業系統環境變數設定：
...
```console
../.venv/Scripts/python.exe -m app.spider_forge_system run --url "https://example.com/news"
```
...
../.venv/Scripts/python.exe -m app.spider_forge_system batch cnyes cna
```

查看執行紀錄與資料位置：

```console
../.venv/Scripts/python.exe -m app.spider_forge_system status
../.venv/Scripts/python.exe -m app.spider_forge_system paths
```
...
```console
../.venv/Scripts/python.exe -m app.spider_forge_system.tests.manual.run_one_stage prepare --input app/spider_forge_system/tests/manual/rba_request.json --output app/spider_forge_system/runtime/manual/rba/01_prepare.json
../.venv/Scripts/python.exe -m app.spider_forge_system.tests.manual.run_one_stage recon --input app/spider_forge_system/runtime/manual/rba/01_prepare.json --output app/spider_forge_system/runtime/manual/rba/02_recon.json
../.venv/Scripts/python.exe -m app.spider_forge_system.tests.manual.run_one_stage triage --input app/spider_forge_system/runtime/manual/rba/02_recon.json --output app/spider_forge_system/runtime/manual/rba/03_triage.json
../.venv/Scripts/python.exe -m app.spider_forge_system.tests.manual.run_one_stage strategy --input app/spider_forge_system/runtime/manual/rba/03_triage.json --output app/spider_forge_system/runtime/manual/rba/04_strategy.json
../.venv/Scripts/python.exe -m app.spider_forge_system.tests.manual.run_one_stage evidence --input app/spider_forge_system/runtime/manual/rba/04_strategy.json --output app/spider_forge_system/runtime/manual/rba/05_evidence.json
../.venv/Scripts/python.exe -m app.spider_forge_system.tests.manual.run_one_stage generate --input app/spider_forge_system/runtime/manual/rba/05_evidence.json --output app/spider_forge_system/runtime/manual/rba/06_generate.json
../.venv/Scripts/python.exe -m app.spider_forge_system.tests.manual.run_one_stage preflight --input app/spider_forge_system/runtime/manual/rba/06_generate.json --output app/spider_forge_system/runtime/manual/rba/07_preflight.json
../.venv/Scripts/python.exe -m app.spider_forge_system.tests.manual.run_one_stage fixture --input app/spider_forge_system/runtime/manual/rba/07_preflight.json --output app/spider_forge_system/runtime/manual/rba/08_fixture.json
```
...
```console
uv pip install --python ../.venv/Scripts/python.exe -r app/spider_forge_system/requirements-training.txt
../.venv/Scripts/python.exe -m app.spider_forge_system train-topic topic_gold.jsonl --output candidate.json
```
...
```console
cd workspace
docker compose --profile manual build crawler-runtime
