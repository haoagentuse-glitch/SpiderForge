...
本專案的重點不是「讓大型語言模型寫爬蟲」，而是前期偵查與**產出之後的驗證鏈**：先用探測到的真實證據限縮模型
能猜的範圍，產碼後用五道閘門逐一擋，失敗時帶著具體錯誤重寫，
...
flowchart TD
    ...
    subgraph LOOP["前期偵查子迴圈"]
        ...
        c2{"檢查二：樣本是真文章<br/><small>程式：有標題、有內文、有發佈時間<br/>兩篇不能雷同</small>"}:::prog
        c3{"檢查三：翻頁有效<br/><small>程式：實抓第二頁要有新文章<br/>捲動則看連結數有沒有增加</small>"}:::prog
        pick --> c1 --> c2 --> c3
    end
...
| 進不進得去 | 程式 | 判斷狀態碼=200，與證據數量 |
| 選抓法 | 程式 | 由一成本低 -> 高測試 |
| 找文章連結 | 程式 ＋ 模型 | 排除導覽是結構事實；分辨標題與分類名要語意 |
| 樣本驗證 | 程式 | 兩篇內容雷同就是拿到列表頁；沒有發佈時間多半是分類頁 |
| 翻頁驗證 | 程式 | 第二頁有沒有新文章?（捲動則看連結數有沒有增加）|
...
診斷會先比對已知的失敗樣態（契約違反、抽取不到、被擋、時效不符），對得上就直接歸類，對不上才問模型。
分類決定路由：可修的進修復迴圈，不可修的直接寫待處理紀錄。
...
某次測試樣本抓成了導覽頁，模型照著錯的樣本學抽取規則，造成兩輪修復都失敗，因為修復用的是同一份錯誤偵查證據。
所以前期偵查改為子迴圈：證據不對就換一種抓法重來，而不是直接往下傳遞。

### 界線只有兩條

不繞登入資料、不造成伺服器負擔（低速、併發固定為一、翻頁有上限）。
...
210 個測試，全部離線：不連網站、不呼叫模型、不消耗額度。測試會把產物寫到暫存目錄，
不碰真實的執行紀錄。**跑完只要一秒多**——這不只是為了快：某次改動讓偵查子迴圈在
測試裡真的去開瀏覽器連網，全套從 1.5 秒變成 13 秒，是那個數字先露的餡。
...
上面那張圖就是現在跑的流程，沒有「尚未實作」的格子。

前期偵查子迴圈（抓法階梯、樣本驗證、捲動偵測）於 2026-08-12 完成。BBC 全流程實跑：
**24 篇、22 篇合格、去重後 22 篇，升版成功**；過程中第一種抓法（直接連線）就被樣本驗證
擋下——BBC 的文章內文是前端渲染的，純 HTTP 只拿得到幾十個字——換成瀏覽器抓法才過關。
這正是這個子迴圈存在的理由：以前這種情況會一路送到產碼，然後被歸成「selector 寫錯」。

每個決定的理由與實測數字在 [`pipelines/GRAPH.md`](pipelines/GRAPH.md)。
